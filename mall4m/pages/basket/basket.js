...
Page({
    ...
    onShow: function () {
        ...
        //加载购物车
        var params = {
            url: "/cart-service/p/shopCart/info",
            method: "GET",
            data: {},
            callBack: res => {
                if (res.length > 0) {
                    // 默认全选
                    var shopCartItem = res[0].shopCartItems;
                    shopCartItem.forEach(shopCartItem => {
                ...
                //         shopCartItems.forEach(shopCartItem opCartItemDiscount.sh=> {
                //             shopCartItem.checked = true;
                //         })
                //     })
                //
    ...
    onSelAll: function () {
        ...
        for (var i = 0; i < shopCartItemDiscounts.length; i++) {
            console.log(shopCartItemDiscounts[i].shopCartItems)
    ...
    calTotalPrice: function () {
        ...
        var params = {
            url: "/cart-service/p/shopCart/totalPay",
            method: "POST",
            data: shopCartIds,
    ...
    onCountMinus: function (e) {
        ...
        var scindex = e.currentTarget.dataset.scindex;
        var shopCartItemDiscounts = this.data.shopCartItemDiscounts;
        var prodCount = shopCartItemDiscounts[scindex].shopCartItems[index].basketCount;
        if (prodCount > 1) {
    ...
    updateCount: function (shopCartItemDiscounts, scindex, index, prodCount) {
        ...
        var params = {
            url: "/cart-service/p/shopCart/changeItem",
            method: "POST",
            data: {
                basketCount: prodCount,
            ...
            callBack: function (res) {
                shopCartItemDiscounts[scindex].shopCartItems[index].basketCount += prodCount;
                ths.setData({
                    shopCartItemDiscounts: shopCartItemDiscounts
                });
            ...
            wx.showModal({
                ...
                success(res) {
                    if (res.confirm) {
                        ...
                        var params = {
                            url: "/cart-service/p/shopCart/deleteItem",
                            method: "DELETE",
                            data: basketIds,
                            callBack: function (res) {
                                wx.hideLoading();
